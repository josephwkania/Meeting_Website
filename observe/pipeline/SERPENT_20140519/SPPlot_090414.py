...
def mergepdfs(tmp, bsl, amporphas='A'):
	...
	if len(tmp) == 1:
		print "No merging recquired - only one plotfile"
			...
			if len(amps[0]) < timeperpage:
				...
				for timestamp in xrange(0,len(amps[0]),timeperpage):
					if len(amps[0]) < timestamp+timeperpage:
						plotname = makespecplot(amps[:,timestamp:timestamp+timeperpage,:], phas[:,timestamp:timestamp+timeperpage,:], weig, time[timestamp:timestamp+timeperpage], bsl, srcname, stokes[st], plfilename+'_'+str(srcname)+'_'+str(timestamp)+'-'+str(len(amps[0])),amporphas,IF_start,IF_end)
						prtfiles.append(plotname)
					else:
						plotname = makespecplot(amps[:,timestamp:timestamp+timeperpage,:], phas[:,timestamp:timestamp+timeperpage,:], weig, time[timestamp:timestamp+timeperpage], bsl, srcname, stokes[st], plfilename+'_'+str(srcname)+'_'+str(timestamp)+'-'+str(timestamp+timeperpage),amporphas,IF_start,IF_end)
						prtfiles.append(plotname)
...
def makespecplotsingle(datachntime,time,weights,bsl,srcname='None',plfilename='None',IF_start=1,IF_end=8,textstrg=[],fixr=[[-1E9,1E9],[0,0],3],statstype='norm',tlabspace=15.0):
	...
	from matplotlib.pyplot import figure, close, savefig, rcParams, cm, cla, clf
	...
	### Create the x-axis (time) labels [day,hour,min,secs]
	tlab = []
	...
	for IFs in xrange(noIFs):
		try:
			vmin = dmean[IFs] - (abs(sigma)*rms[IFs])
			vmax = dmean[IFs] + (abs(sigma)*rms[IFs])
			vvmin.append(vmin)
			vvmax.append(vmax)
		except IndexError:
	...
	for IFs in xrange(IF_start, IF_end+1):
		...
		image = tmp.imshow(datachntime[IFs-1],aspect='auto',interpolation='nearest',origin='lower',cmap=cm.spectral,vmin=vvmin[count-1],vmax=vvmax[count-1])
	...
	cbar = fig1.colorbar(image, cax=cax)
	cbar.ax.tick_params(labelsize=fsize)

	if str(dstype).count('amp') or str(dstype).count('AMP') > 0:
		bottomtext = str(srcname)+' '+dstype+' BSL ['+str(bsl[0])+':'+str(bsl[1])+'] Stoke:'+str(stokes)+' Mean: '+str('%.5f'%avdmean)+' RMS: '+str('%.5f'%avrms)+' Max: '+str('%.5f'%avmax)+' Jy'
	...
	if str(dstype).count('pha') or str(dstype).count('PHA') > 0:
		bottomtext = str(srcname)+' '+dstype+' BSL ['+str(bsl[0])+':'+str(bsl[1])+'] Stoke:'+str(stokes)+' Mean: '+str('%.5f'%avdmean)+' RMS: '+str('%.5f'%avrms)+' Max: '+str('%.5f'%avmax)+' deg'
	...
	if str(dstype).count('nvis') or str(dstype).count('NVIS') > 0:
		bottomtext = str(srcname)+' '+dstype+' BSL ['+str(bsl[0])+':'+str(bsl[1])+'] Stoke:'+str(stokes)+' Mean: '+str('%.5f'%avdmean)+' RMS: '+str('%.5f'%avrms)+' Max: '+str('%.5f'%avmax)+' counts'
	...
	fig1.text(0.5, 0.03, bottomtext, horizontalalignment='center')
	fig1.suptitle(plfname, fontsize=fsize*2)
...
if flagver == 0:
	...
	if len(specifybaselines) > 5:
		...
		picklefilename = picklepath+"/"+str(outfilename)+"-"+str(Name)+"_FG:"+str(flagver)+'_'+str(outclass)+"_dic.p"
...
if flagver >= 1:
	if specifybaselines <= 5:
		...
		dbasebsl, dbasebslhead = datatobase(uvbsldatav,outfilename,fagver,True,False)
